...
function ig_CreateComboBox(id)
{
	...
	{ 
		...
		while(html.tagName != 'HTML') html = html.nextSibling;
		
	    var head = html.childNodes[0];
	    if(head.tagName != "BODY")
	    {
	        while(head.tagName != 'HEAD')
	            head = head.nextSibling;
	    }
	    else
	    {
	        /* Safari - Need to rebuild the html document, first remove the body. 
	         * Then add a new head element, and then add a body element*/
...
var AM = ComboStrings[0];
var PM = ComboStrings[1];
var COLON = ":";
if(MilitaryTime)
{
	AM = "";
...
{
    /* SJZ BR22635 8/10/07  Need to check if we're in military time */ 
    if(MilitaryTime)
    {
	    AM = "";
	    PM = "";
...
function convertStringToDateTime(value)
{
	...
	if(!MilitaryTime)
	{
		var ttindex = value.indexOf(ComboStrings[0]);
