...
function InitializeValues()
{
	...
	if(!dialogInterface.getAllowAllDayEvents())	
	{
	    ...
	    /* SJZ 1/11/08 - looks like the id of the label changed*/
...
function HideTBarButton(key)
{
    ...
    if (colGroup.tagName != "COLGROUP")
    {
        ...
        for (var j = 0; j < colGroup.childNodes.length; j++)
        {
            ...
            {
                /* VS 05/22/2013 Bug 142807 do not use display:none */
                colGroup.childNodes[j].style.width = "0px";
                colGroup.childNodes[j].style.visibility = "hidden";
                break;
            }
        }
        /* VS 05/22/2013 Bug 142807 do not exclude IE and do not use display:none */
        button.Element.style.width = "0px";
...
{	
	if(oButton.Key != "High" && oButton.Key != "Low" )
	{
		...
		var activityEditProps = {StartDate:		 {	Element: startDateChooser.Element, 
			...
			if(variance)
			{
				...
				{
				    /*SJZ 1/11/08 BR29590 - previously we were actually overwriting the recurrenceEndDate, and thats a bad thing*/
	...
	if(oButton.Index == 3) // Recurrence
	{
		...
		/* D.M. BR25712 3/5/2008 - Using settimeout here to allow the recurrence button to
		 * lose focus while the dialog is opening. */
