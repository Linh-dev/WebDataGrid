...
public partial class Samples_WebCharts_Display_Visual_Appearance_Office_2007_Styles_Default : Infragistics.Web.SampleBrowser.Core.Framework.Web.UI.ASPNET.SamplePage
{
    ...
    private const string virtualPath = "/SamplesBrowser/SamplesCommon/aspnet/WebCharts/Display/VisualAppearance/2007/XML";

    private void LoadPreset(string preset)
    {
        string theChartTypeString = preset.Substring(10, preset.IndexOf("Style") - 10);
        ChartType theChartType;
        switch (theChartTypeString)
        {
            case "Pie":
                theChartType = ChartType.PieChart;
                break;
            case "Line":
                theChartType = ChartType.LineChart;
                break;
            case "Scatter":
                theChartType = ChartType.ScatterChart;
                break;
            case "StackedBarColumn":
                theChartType = ChartType.StackColumnChart;
                break;
            default:
            case "BarColumn":
                theChartType = ChartType.ColumnChart;
                break;
        }
        this.UltraChart1.ChartType = theChartType;

        string filePath = string.Format("{0}/{1}.xml", Server.MapPath(virtualPath), preset);
        try
        {
            this.UltraChart1.LoadPreset(filePath, true);
        }
        catch (System.IO.IOException)
        {
            Response.Write("Error: unable to load preset " + filePath + ".  Please check the file location and verify that it exists and can be opened.");
        }
    ...
    private void Page_Load(object sender, System.EventArgs e)
    {
        ...
        if (!Page.IsPostBack)
        {
            FileInfo[] fis = new DirectoryInfo(Server.MapPath(virtualPath)).GetFiles("Office2007*.xml");
            foreach (FileInfo fi in fis)
            {
                this.ListBox1.Items.Add(fi.Name.Substring(0, fi.Name.Length - 4));
            }
            this.ListBox1.Items[0].Selected = true;
            this.LoadPreset("Office2007BarColumnStyle17");
        }

        this.ListBox1.Visible = this.CheckBox1.Checked;
        this.presetPicker.Visible = !this.CheckBox1.Checked;

        if (this.ListBox1.Visible)
        {
            this.LoadPreset(this.ListBox1.SelectedItem.ToString());
        }
    ...
    protected void ImageButtons_OnClick(object sender, Infragistics.WebUI.WebDataInput.ButtonEventArgs e)
    {
        string preset;
        if (sender == this.AquaButton)
        {
            preset = "Office2007BarColumnStyle17";
        }
        else if (sender == this.ShadesOfBlueButton)
        {
            preset = "Office2007BarColumnStyle13";
        }
        else if (sender == this.OfficialRedButton)
        {
            preset = "Office2007BarColumnStyle32";
        }
        else if (sender == this.GreenGradientButton)
        {
            preset = "Office2007BarColumnStyle21";
        }
        else //if (sender == this.OrangeAndBlackButton)
        {
            preset = "Office2007BarColumnStyle54";
        }
