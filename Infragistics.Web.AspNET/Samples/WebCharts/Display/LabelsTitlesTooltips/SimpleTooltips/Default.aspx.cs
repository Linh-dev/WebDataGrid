...
public partial class Samples_WebCharts_Display_Labels_Titles_Tooltips_Simple_Tooltips_Default : Infragistics.Web.SampleBrowser.Core.Framework.Web.UI.ASPNET.SamplePage
{
    ...
    private void Page_Load(object sender, System.EventArgs e)
    {
        ...
        if (!Page.IsPostBack)
        {
            string[] tooltipStyle = System.Enum.GetNames(typeof(TooltipStyle));
            foreach (string s in tooltipStyle)
            {
                this.DropDownList1.Items.Add(s);
            }

            string[] tooltipDisplay = System.Enum.GetNames(typeof(TooltipDisplay));
            foreach (string s in tooltipDisplay)
            {
        ...
        this.UltraChart1.Tooltips.Display = (TooltipDisplay)System.Enum.Parse(typeof(TooltipDisplay), this.DropDownList2.SelectedItem.Value);
        this.UltraChart1.Tooltips.Format = (TooltipStyle)System.Enum.Parse(typeof(TooltipStyle), this.DropDownList1.SelectedItem.Value);
        if (this.DropDownList1.SelectedItem.Value == "Custom")
        {
            this.UltraChart1.Tooltips.FormatString = "My Data: <DATA_VALUE:$#0.00>";
        }
    }
